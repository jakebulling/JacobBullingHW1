...
```{r}
hotels_2 <- hotels %>%
  transform(is_canceled = if_else(is_canceled !=0, TRUE, FALSE))
...
ggplot(hotels_2) + 
  aes(x = is_canceled, y =lead_time) + 
  geom_boxplot()


ggplot(hotels_2) + 
  aes(x = booking_changes, fill = is_canceled) + 
...
set.seed(494)
df <- sort(sample(nrow(hotels_mod), nrow(hotels_mod) * .5))

train_data<- hotels_mod[df,]
test_data<- hotels_mod[-df,]
